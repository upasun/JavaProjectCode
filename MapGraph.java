...
public class MapGraph {
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		return algoSearch(start, goal, nodeSearched, astar);
	}
	
	public List<GeographicPoint> algoSearch(GeographicPoint start, 
			  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched, GeographicPoint astar)
	{
		...
		Comparator<Nodes<GeographicPoint>> myComp = new PQcomp(myT);
		PriorityQueue<Nodes<GeographicPoint>> myQ = new PriorityQueue<Nodes<GeographicPoint>>(11, myComp);
		myQ.add(myT.getNode(start));
		//System.out.println("");
		GeographicPoint temp = start;
		...
		while((temp != null) && (!myQ.isEmpty() || (temp.getX() != goal.getX()) || (temp.getY() != goal.getY()))){
			tNode = myQ.poll(); //find node being explored in SearchTree
			temp = tNode.getData();
			nodeSearched.accept(tNode.getData());
			//System.out.println("hi");
			...
			if(!set.contains(temp)){
				set.add(temp);
				ArrayList<GeographicPoint> theL = theGraph.get(tNode.getData());
				if(theL != null){
					//System.out.println("Node is:  " + tNode.getData());
					for(int i = 0; i < theL.size(); i++){
						GeographicPoint gp = theL.get(i);
						//System.out.println("      containing:  " + gp);
						//The search tree contains a hashmap in which I can check for the existence of a certain GeographicPoint
						//So if a connection has not been explored
