...
public class NearbyWords implements SpellingSuggest {
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(queue.size() > 0 && retList.size() < numSuggestions && check < THRESHOLD){
			String s = queue.poll();
			List<String> newWords = distanceOne(s, true);
			visited.add(s);
			for(int i = 0; i < newWords.size(); i++){
				if(!visited.contains(newWords.get(i))){
					retList.add(newWords.get(i));
					queue.add(newWords.get(i));
				}
			}
			check++;
		}
		if(retList.size() > numSuggestions){
			retList = retList.subList(0, numSuggestions);
		}
